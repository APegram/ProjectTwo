$(function() {
  ...
  var instelem = document.getElementById("adminChat");
    ...
    if (data.exerciseName != exerciseName) {
      ...
      $.ajax({
        ...
        if (results[0].stepOne !== null) {
          ...
          var paragraph = $("<p>");
          var label = $(
            "<label class='stepComplete btn btn-primary btn-rounded btn-sm' data-toggle='buttons'>"
          );
  ...
  socket.on("newChat", function(data) {
    ...
    var outerSpan = $("<span>");
    var chatDiv = $(
      "<div class='chip chip-md peach-gradient darken-2 white-text'>"
    );
    var chatSpan = $("<span>" + data.text + "</span>");
    chatDiv.append(
      "<img src='../img/images/einstein.jpg' alt='" +
        data.userName +
        "'>" +
        data.userName
    );
